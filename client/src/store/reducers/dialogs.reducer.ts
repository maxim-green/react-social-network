    ...
    case 'rsn/chat/MARK_MESSAGE_AS_READ': {
      return {
        ...
        messages: state.messages.map(
          (message) => (message._id !== action.messageId ? message : { ...message, isRead: true }),
        ),
      };
    }
...
export const dialogsActions = {
  ...
  markMessageAsRead: (messageId: string) => ({
    type: 'rsn/chat/MARK_MESSAGE_AS_READ',
    messageId,
  } as const),
  setCachedMessage: (text: string, dialogId: string) => ({
    type: 'rsn/chat/SET_CACHED_MESSAGE',
    text,
    dialogId,
  } as const),
...
const _connectHandler: (() => void) | null = null;
const connectHandlerCreator = (
  dispatch: Dispatch<DialogsActionType | ThunkType<DialogsActionType>>,
  getState: () => RootState,
) => {
...
const _messageHandler: ((message: MessageType) => void) | null = null;
...
const _unreadMessagesHandler: ((unreadMessagesCount: number) => void) | null = null;
...
const _notAuthorizedHandler: (() => null) | null = null;
const notAuthorizedHandlerCreator = (
...
export const readMessage = (
  messageId: string,
): ThunkType<DialogsActionType> => async (dispatch) => {
