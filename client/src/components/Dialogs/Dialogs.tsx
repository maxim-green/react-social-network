...
export const Dialogs: React.FC<PropsType> = ({
  messages, dialogs, authUser, onNewMessageSubmit, currentCompanion,
}) => {
      ...
      <div className={classnames(classes.dialogs, { [classes.listActive]: listActive })}>
        ...
        <div className={classes.header}>
          <div className={classes.dialogsListSwitch}>
            {!listActive && (
            <Button type="text" size="xl" onClick={showDialogsList}>
              <Button.Icon><List width={22} height={22} /></Button.Icon>
            </Button>
            )}
            {listActive && (
            <Button type="text" size="xl" onClick={hideDialogsList}>
