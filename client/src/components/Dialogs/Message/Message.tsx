...
export const Message: React.FC<Props> = ({ message, authUser }) => {
  ...
  // todo think better way to read message. for example send 'read-message' event and handle it on
  // server
      ...
      <div className={isAuthor ? classes.messageSelf : classes.messageOther}>
        <Row gap={10}>
          <div className={classes.messageAvatar}>
            <Avatar
              online={checkOnline(message.author.updatedAt)}
              smallImg={message.author.avatar.small}
              size={30}
            />
          </div>
          <div>
