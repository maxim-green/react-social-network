...
import {
  closeDialog, getDialogs, openDialog, sendMessage,
} from 'store/reducers/dialogs.reducer';
...
export const DialogsPage: React.FC = () => {
  ...
  const currentCompanion = dialogs.find(
    (dialog) => dialog.companion.username === username,
  )?.companion;
  ...
  if (!username && dialogs[0] && dialogs.length !== 0) {
    return (
      <Redirect
    ...
    <>
      {currentCompanion && (
        <Helmet>
          <title>
            Dialog -
            {currentCompanion.firstName}
            {' '}
            {currentCompanion.lastName}
          </title>
        </Helmet>
      )}
      <Dialogs
